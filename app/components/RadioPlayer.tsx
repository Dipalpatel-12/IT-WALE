...
const PROGRESS_KEY = "it-wale-player-progress";
...
function readSavedProgress(playlistId: string): SavedProgress | null {
    try {
        const raw = localStorage.getItem(PROGRESS_KEY);
        if (!raw) return null;
        const parsed = JSON.parse(raw) as SavedProgress;
        if (parsed?.playlistId === playlistId) return parsed;
        return null;
    } catch {
        return null;
    }
...
function saveProgress(playlistId: string, index: number, time: number) {
    try {
        localStorage.setItem(PROGRESS_KEY, JSON.stringify({ playlistId, index, time }));
    } catch {}
}
...
export default function RadioPlayer({ playlistId, categoryTitle }: RadioPlayerProps) {
    ...
    useEffect(() => {
        ...
        const saved = readSavedProgress(playlistId);
        const startIndex = saved?.index ?? 0;
        const startSeconds = saved?.time ?? 0;
        ...
        function createPlayer() {
            playerRef.current = new window.YT.Player("yt-player-" + playlistId, {
                ...
                playerVars: {
                    ...
                    list: playlistId,
                    loop: 1,
                    // autoplay NAHI diya — refresh pe khud se play nahi hoga
                },
                events: {
                    onReady: (e: any) => {
                        e.target.setVolume(volume);
                        // Sahi song aur sahi time pe LOAD karo, par play mat karo
                    ...
                    onStateChange: (e: any) => {
                        ...
                        if (e.data === S.PLAYING) {
                            setIsPlaying(true);
                            syncVideoInfo();
                            currentIndexRef.current = playerRef.current.getPlaylistIndex?.() ?? currentIndexRef.current;
                        } else if (e.data === S.PAUSED) {
                            setIsPlaying(false);
        ...
        intervalRef.current = setInterval(() => {
            if (playerRef.current?.getCurrentTime && !isSeeking) {
                ...
                currentIndexRef.current = playerRef.current.getPlaylistIndex?.() ?? currentIndexRef.current;
                saveProgress(playlistId, currentIndexRef.current, t);
            }
        }, 1000);

        return () => {
            if (intervalRef.current) clearInterval(intervalRef.current);
            saveProgress(playlistId, currentIndexRef.current, currentTimeRef.current);
            playerRef.current?.destroy?.();
        };
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [playlistId]);

    // Refresh/tab-close se theek pehle bhi progress save
    useEffect(() => {
