...
export default function SuggestionPrinter() {
    ...
    const handleSubmit = async (e: React.MouseEvent) => {
        ...
        if (!message.trim()) {
            gsap.to(".suggestion-textarea", { borderColor: "#E05A5A", duration: 0.15 });
            setTimeout(() => {
                gsap.to(".suggestion-textarea", { borderColor: "#E5E1D6", duration: 0.3 });
            }, 600);
            return;
        }
    ...
    return (
        ...
        <div className="flex flex-col items-center">
            ...
            <div
                className="relative w-[260px] h-[340px] rounded-[26px] border overflow-hidden"
                style={{
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <input
                            ...
                            className="text-[17px] px-2.5 py-2 rounded-[7px] text-black border border-[#E5E1D6] outline-none focus:border-sky-400"
                        />
                        <textarea
                            placeholder="Your Suggestion..."
                            value={message}
                            onChange={(e) => setMessage(e.target.value)}
                            className="suggestion-textarea text-[17px] px-2.5 py-2 text-black rounded-[7px] border border-[#E5E1D6] outline-none resize-none min-h-[56px] focus:border-sky-400"
                        />
                        <button
                            onClick={handleSubmit}
                            disabled={submitting}
                            className="mt-0.5 bg-[#1A1109] text-[#f0a94e] rounded-[7px] py-1.5 text-[11.5px] font-bold tracking-wide disabled:opacity-50 cursor-pointer"
                        >
                ...
                <button
                    ref={tapBtnRef}
                    onClick={printOut}
                    className="absolute top-[150px] left-1/2 -translate-x-1/2 w-[116px] h-[116px] rounded-full border-none cursor-pointer flex items-center justify-center z-[2]"
                    style={{
